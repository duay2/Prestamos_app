...
class ReporteDialog:
    ...
    def crear_widgets(self):
        ...
        ttk.Label(main_frame, text=f"Configurar {self.tipo_reporte}", 
...
class ReportesWindow(ttk.Frame):
    ...
    def crear_widgets(self):
        ...
        title_label = ttk.Label(main_frame, text="📊 Reportes y Estadísticas", 
                               font=('Segoe UI', 18, 'bold'), 
                               foreground='#1E3A5F')
        title_label.pack(pady=(0, 20))
        
        # Frame para botones de reportes - usar Frame normal para evitar fondo marrón
        reports_container = tk.Frame(main_frame, bg='#F5F7FA')
        reports_container.pack(fill='x', pady=(0, 20))
        
        # Título de la sección
        section_title = tk.Label(reports_container, text="Tipos de Reporte", 
                                bg='#F5F7FA', fg='#1E3A5F', 
                                font=('Segoe UI', 10, 'bold'))
        section_title.pack(anchor='w', padx=10, pady=(10, 5))
        
        # Frame interno para botones con fondo blanco
        reports_frame = tk.Frame(reports_container, bg='white', relief='flat', bd=1)
        reports_frame.pack(fill='x', padx=10, pady=(0, 10))
        
        # Botones de reportes
        ttk.Button(reports_frame, text="📋 Estado de Cuenta", 
                  command=lambda: self.generar_reporte("Estado de Cuenta")).pack(fill='x', pady=5, padx=10)
        
        ttk.Button(reports_frame, text="💰 Resumen de Ingresos", 
                  command=lambda: self.generar_reporte("Resumen de Ingresos")).pack(fill='x', pady=5, padx=10)
        
        ttk.Button(reports_frame, text="⏰ Préstamos Próximos a Vencer", 
                  command=lambda: self.generar_reporte("Préstamos Próximos a Vencer")).pack(fill='x', pady=5, padx=10)
        
        ttk.Button(reports_frame, text="📅 Reporte Mensual", 
                  command=lambda: self.generar_reporte("Reporte Mensual")).pack(fill='x', pady=5, padx=10)
        
        # Frame para vista previa - usar Frame normal para evitar fondo marrón
        preview_container = tk.Frame(main_frame, bg='#F5F7FA')
        preview_container.pack(fill='both', expand=True)
        
        # Título de la sección
        preview_title = tk.Label(preview_container, text="Vista Previa", 
                                bg='#F5F7FA', fg='#1E3A5F', 
                                font=('Segoe UI', 10, 'bold'))
        preview_title.pack(anchor='w', padx=10, pady=(0, 5))
        
        # Frame interno para el texto con fondo blanco
        preview_frame = tk.Frame(preview_container, bg='white', relief='flat', bd=1)
        preview_frame.pack(fill='both', expand=True, padx=10, pady=(0, 10))
        
        # Texto de vista previa con fondo blanco
        self.preview_text = tk.Text(preview_frame, height=20, width=80, state='disabled',
                                    bg='white', fg='#1E3A5F', font=('Consolas', 10))
