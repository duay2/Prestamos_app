...
class ClienteModel:
    ...
    def agregar_cliente(self, nombre: str, telefono: str, direccion: str) -> int:
        ...
        try:
            ...
            # Insertar el nuevo cliente (se permite teléfono duplicado)
            cursor.execute('''
    ...
    def actualizar_cliente(self, cliente_id: int, nombre: str, telefono: str, direccion: str) -> bool:
        ...
        try:
            ...
            # Actualizar el cliente (se permite teléfono duplicado)
            cursor.execute('''
                UPDATE clientes 
...
class ClienteDialog:
    ...
    def crear_widgets(self):
        ...
        ttk.Label(main_frame, text=titulo, font=('Segoe UI', 14, 'bold'), foreground='#1E3A5F').pack(pady=(0, 20))
    ...
    def verificar_duplicado(self, nombre: str, telefono: str) -> bool:
        ...
        try:
            ...
            # Nota: Se permite teléfono duplicado, solo se verifica nombre
            cursor.execute('''
                SELECT id FROM clientes 
                WHERE UPPER(TRIM(nombre)) = UPPER(TRIM(?))
            ''', (nombre,))
...
class ClientesWindow(ttk.Frame):
    ...
    def crear_widgets(self):
        ...
        # Título con estilo mejorado usando colores del logo
        ...
        # Filtros avanzados - usar Frame normal en lugar de LabelFrame para evitar colores no deseados
        filter_frame = tk.Frame(top_frame, bg='white', relief='flat', bd=1)
        filter_frame.pack(side='left', fill='x', expand=True, padx=5, pady=5)
        
        # Título de la sección con colores del logo
        title_label = tk.Label(filter_frame, text="Búsqueda y Filtros", bg='white', fg='#1E3A5F', font=('Segoe UI', 10, 'bold'))
        ...
        tk.Label(filter_row1, text="Buscar por:", bg='white', fg='#1E3A5F', font=('Segoe UI', 9)).pack(side='left', padx=(0, 5))
        self.tipo_busqueda = tk.StringVar(value="nombre")
        ttk.Radiobutton(filter_row1, text="Nombre", variable=self.tipo_busqueda, value="nombre", style='Filter.TRadiobutton').pack(side='left', padx=(0, 10))
        ttk.Radiobutton(filter_row1, text="Teléfono", variable=self.tipo_busqueda, value="telefono", style='Filter.TRadiobutton').pack(side='left', padx=(0, 10))
        ttk.Radiobutton(filter_row1, text="Dirección", variable=self.tipo_busqueda, value="direccion", style='Filter.TRadiobutton').pack(side='left')
        
        # Segunda fila de filtros - usar Frame de tkinter normal
        filter_row2 = tk.Frame(filter_content, bg='white')
        filter_row2.pack(fill='x')
        
        tk.Label(filter_row2, text="Término:", bg='white', fg='#1E3A5F', font=('Segoe UI', 9)).pack(side='left', padx=(0, 5))
        ...
        # Botón destacado para Agregar (dorado)
        ttk.Button(button_frame, text="➕ Agregar", command=self.agregar_cliente, style='Accent.TButton').pack(side='left', padx=5)
